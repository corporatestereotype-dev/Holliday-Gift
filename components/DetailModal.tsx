...
const HomotopyVisualizer: React.FC<{ text: string }> = ({ text }) => {
    // 1. Check if it's numbered list
    const lines = text.split('\n').map(l => l.trim()).filter(l => l);
    // Heuristic: At least 2 lines start with a number digit
    const numberedLines = lines.filter(l => /^\d+[\.:\)]/.test(l));
    const isStepList = numberedLines.length >= 2 && numberedLines.length / lines.length > 0.5;

    if (isStepList) {
         return (
             <div className="relative my-2 pl-2">
                 {/* Vertical line connecting nodes */}
                 <div className="absolute left-[15px] top-4 bottom-4 w-0.5 bg-gradient-to-b from-rose-500/30 via-cyan-500/30 to-emerald-500/30" />
                 
                 <div className="space-y-4">
                     {lines.map((line, idx) => {
                         const match = line.match(/^(\d+)[\.:\)]\s*(.*)/);
                         // If line doesn't match number pattern but we determined it is a list, treat it as continuation or unnumbered step
                         ...
                         let circleBorder = "border-slate-600";
                         let circleText = "text-slate-500";
                         let cardBg = "bg-slate-800 border-slate-700 text-slate-300";

                         if (isFirst) {
                             circleBorder = "border-rose-500";
                             circleText = "text-rose-400";
                             cardBg = "bg-rose-500/5 border-rose-500/20 text-rose-100";
                         } else if (isLast) {
                             circleBorder = "border-emerald-500";
                             circleText = "text-emerald-400";
                             cardBg = "bg-emerald-500/5 border-emerald-500/20 text-emerald-100";
                         } else {
                             circleBorder = "border-cyan-500";
                             circleText = "text-cyan-400";
                             cardBg = "bg-slate-800 border-slate-700 text-slate-300";
                         }

                         return (
                             <div key={idx} className="relative flex items-start gap-4">
                                 {/* Node */}
                                 <div className={`flex-shrink-0 w-8 h-8 rounded-full border-2 bg-slate-900 flex items-center justify-center z-10 font-bold text-xs ${circleBorder} ${circleText}`}>
                                     {stepNum}
                                 </div>
                                 
                                 {/* Card */}
                                 <div className={`flex-grow p-3 rounded-lg border text-sm shadow-sm ${cardBg}`}>
                    ...
                    {homotopyPathContent && (
                        <div className="mt-6 bg-slate-900/30 border border-slate-700/50 rounded-xl p-6 relative overflow-hidden group">
                            ...
                            <div className="flex items-start gap-4 relative z-10">
                                {/* Side bar for visual context of start/end */}
                                <div className="hidden sm:flex flex-col items-center mt-3 mr-2">
                                    <div className="w-2 h-2 rounded-full bg-rose-500 shadow-lg shadow-rose-500/50"></div>
                                    <div className="w-px h-full bg-gradient-to-b from-rose-500 via-slate-600 to-emerald-500 my-1 min-h-[40px] opacity-50"></div>
                                ...
                                <div className="space-y-4 flex-1">
                                    <div>
                                        ...
                                        <p className="text-slate-500 text-xs italic mb-2">"Singular configuration, infinite divergence, or logical paradox."</p>
                                    </div>
                                    
                                    <HomotopyVisualizer text={homotopyPathContent} />
